...
import { KatakanaCharacter, katakanaCharacters } from '../models/katakana';
import { speak, stopSpeaking } from '../utils/tts';

interface QuizQuestion {
  character: KatakanaCharacter;
  options: string[];
  correctAnswer: string;
}

function KatakanaQuiz(): React.ReactElement {
  ...
  const generateQuestion = () => {
    ...
    const character = katakanaCharacters[randomIndex];
    
    // Generate 3 wrong options
    let options = [character.romaji];
    while (options.length < 4) {
      const wrongIndex = Math.floor(Math.random() * katakanaCharacters.length);
      const wrongOption = katakanaCharacters[wrongIndex].romaji;
  ...
  // Initialize with a question when component mounts
  useEffect(() => {
    generateQuestion();
  }, []);
  ...
  const pronounceCharacter = async () => {
    ...
    try {
      ...
      if (japaneseVoice) {
        await speak(currentQuestion.character.character, japaneseVoice);
      } else {
